...
`remoteType: "trs80sim"` embeds Lawrence Kesteloot's open-source [TypeScript TRS-80 emulator](https://github.com/lkesteloot/trs80) ([`packages/trs80-emulator`](https://github.com/lkesteloot/trs80/tree/main/packages/trs80-emulator), Copyright © Lawrence Kesteloot, MIT License) in-process in the extension host — the same architecture as DeZog's internal `zsim` simulator. No external emulator, no socket, no ROM hunting required (note: `trszog` specifically requires George Phillips' [zmac](http://48k.ca/zmac.html) macro assembler to generate the `.bds` symbol files used for TRS-80 source mapping).
...
- **[Lawrence Kesteloot](https://github.com/lkesteloot)** wrote the open-source **TypeScript TRS-80 emulator** ([lkesteloot/trs80](https://github.com/lkesteloot/trs80) / [`packages/trs80-emulator`](https://github.com/lkesteloot/trs80/tree/main/packages/trs80-emulator), Copyright © Lawrence Kesteloot, MIT License) that powers the built-in `trs80sim` remote — a beautifully engineered machine: clean HAL architecture, inline ROMs, an instruction-precise CPU core, and the authentic screen renderer you see in the screenshots above. We embedded it **without changing a single line of its code** — the highest compliment an architecture can receive. If you enjoy the zero-setup debugging in this fork, you are enjoying Lawrence's emulator.
